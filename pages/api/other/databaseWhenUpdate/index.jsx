  ...
  if (req.method === "POST") {
    try {
      ...
      if (!data || data.error || data === undefined || data.length === 0) {
        return res
          .status(500)
          .json({ status: "error", error: data.error, data: null });
      }
      ...
      for (let i = 0; i < dataFromDB.length; i++) {
        if (
          reqBodyData.includes(dataFromDB[i].name) &&
          !dataFromDB[i].isNewData
        ) {
      ...
      if (
        ...
        deleteDataFromDb === undefined
      ) {
        return res
          .status(500)
          .json({ status: "error", error: deleteDataFromDb.error, data: null });
      }

      // veri tabanına yeni veri ekliyoruz.
      const createNewDataToDb = await createNewData(
        "dataUpdateChecker",
        readyForSendDB
      );
      ...
      if (
        !createNewDataToDb ||
        createNewDataToDb.error ||
        createNewDataToDb === undefined
      ) {
        return res
          .status(500)
          .json({
            status: "error",
            error: createNewDataToDb.error,
            data: null,
          });
      }
